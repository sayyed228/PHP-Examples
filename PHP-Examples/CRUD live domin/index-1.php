...
<?php
$connect = mysql_connect('webcreativities.net','webcreat','sayyed10217');
...
mysql_select_db('webcreat_panjigroup', $connect);
?>
...
{
echo('<tr>');
echo('<td><span><input class="name" type="text" value="'.$row['TicketTitle'].'" /></span></td>');
/*echo('<td><input class="Phone" type="text" value="'.$row['Phone'].'" /> </td>');
echo('<td><span class="Birthday">'.$row['Birthday'].'</span></td>');
echo('<td><input type="button" id="'.$row['id'].'" value="'.$row['id'].'"/></td>');*/
echo('</tr>');
}
echo('</table>');
?>
...
aa.send()
	...
	var Birthday = $(this).closest('tr').find('.Birthday').text()
	   $('#id').val(id)
		$('#name').val(name)
		$('#Phone').val(Phone)
